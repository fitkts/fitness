...
describe('<MemberNotesForm />', () => {
  ...
  test('수정 모드에서 메모 textarea가 올바르게 렌더링된다', () => {
    ...
    const textarea = screen.getByPlaceholderText(/회원에 대한 특이사항이나 메모를 입력하세요./);
  ...
  test('뷰 모드에서 메모 내용이 올바르게 표시된다', () => {
    ...
    expect(screen.queryByPlaceholderText(/회원에 대한 특이사항이나 메모를 입력하세요./)).not.toBeInTheDocument();
  });

  test('뷰 모드에서 메모가 없을 때 안내 문구가 표시된다', () => {
    render(<MemberNotesForm {...defaultProps} notes={undefined} isViewMode={true} />);
