...
export async function getMemberStats(): Promise<{
  ...
  try {
    ...
    const expiredMembers = db
      .prepare('SELECT COUNT(*) as count FROM members WHERE membership_end < ?')
      .get(today).count;

    // 이번 달 신규 가입 회원 수
    const newMembersThisMonth = db
      .prepare('SELECT COUNT(*) as count FROM members WHERE join_date >= ?')
...
export async function getMembersWithPagination(
  ...
  try {
    ...
    const total = db.prepare(countQuery).get(...params.slice(0, -2)) as {
      total: number;
    };
