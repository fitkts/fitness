...
// SettingsData는 electron.d.ts 내부에서 SettingsData를 임포트하여 사용하므로, Backup.tsx에서 직접 SettingsData를 사용할 일이 없다면 여기서 임포트할 필요는 없습니다.
// 만약 Backup.tsx 내부 로직에서 SettingsData 타입 정보가 직접적으로 필요하다면, 다음 주석을 해제하세요.
// import { SettingsData } from '../types';

// SettingsData 인터페이스 정의는 src/types/index.ts 로 이동되었습니다.
// declare global 블록은 src/types/electron.d.ts 로 이동되었습니다.
...
const Backup: React.FC = () => {
  ...
  const loadBackupFiles = async () => {
    if (!api) {
      showToast(
        'error',
        '백업 API를 사용할 수 없습니다. (preload 설정 확인 필요)',
      );
      setIsLoading(false);
      return;
    }
  ...
  const handleManualBackup = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        showToast(
          'error',
          `백업 생성 실패: ${result.error || '알 수 없는 오류'}`,
        );
      }
    } catch (error) {
  ...
  const handleRestoreConfirm = async () => {
    ...
    try {
      ...
      if (success) {
        showToast(
          'success',
          '백업이 성공적으로 복원되었습니다. 프로그램을 재시작합니다.',
        );
        if (window.api && window.api.relaunchApp) {
          ...
          showToast(
            'warning',
            '프로그램 재시작 API를 사용할 수 없습니다. 수동으로 재시작해주세요.',
          );
          console.warn('Electron API (relaunchApp) is not available.');
        }
      } else {
  ...
  const handleDeleteBackup = async (backup: BackupFile) => {
    if (
      !confirm(
        `'${format(backup.date, 'yyyy-MM-dd HH:mm')}' 백업을 삭제하시겠습니까?`,
      )
    ) {
      return;
    }
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <div className="text-sm text-gray-500">
            마지막 자동 백업:{' '}
            {backupFiles[0]
              ? format(backupFiles[0].date, 'yyyy-MM-dd HH:mm', { locale: ko })
              : '없음'}
          </div>
        </div>
          ...
          <div className="space-y-2 text-gray-600">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-semibold">
                {backupFiles[0]
                  ? formatDistanceToNow(backupFiles[0].date, {
                      addSuffix: true,
                      locale: ko,
                    })
                  : '없음'}
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {backupFiles.map((backup) => (
                  <tr key={backup.path} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {formatDistanceToNow(backup.date, {
                        addSuffix: true,
                        locale: ko,
                      })}
                    </td>
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            ...
            <div className="mb-6">
              <p className="text-red-600 font-medium mb-2">
                주의: 이 작업은 현재 데이터를 백업 데이터로 대체합니다.
              </p>
              <p className="text-gray-600">
                {format(selectedBackup.date, 'yyyy-MM-dd HH:mm', {
                  locale: ko,
                })}{' '}
                시점의 백업으로 복원하시겠습니까? 저장되지 않은 모든 변경사항은
                손실됩니다.
              </p>
            </div>
